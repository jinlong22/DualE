...
class DualE(Model):
    def __init__(self, config):
        super(DualE, self).__init__(config)
        self.emb_1 = nn.Embedding(self.config.num_ent, self.config.emb_dim)
        self.emb_2 = nn.Embedding(self.config.num_ent, self.config.emb_dim)
        self.emb_3 = nn.Embedding(self.config.num_ent, self.config.emb_dim)
        self.emb_4 = nn.Embedding(self.config.num_ent, self.config.emb_dim)
        self.emb_5 = nn.Embedding(self.config.num_ent, self.config.emb_dim)
        self.emb_6 = nn.Embedding(self.config.num_ent, self.config.emb_dim)
        self.emb_7 = nn.Embedding(self.config.num_ent, self.config.emb_dim)
        self.emb_8 = nn.Embedding(self.config.num_ent, self.config.emb_dim)
        self.rel_1 = nn.Embedding(self.config.num_rel, self.config.emb_dim)
        self.rel_2 = nn.Embedding(self.config.num_rel, self.config.emb_dim)
        self.rel_3 = nn.Embedding(self.config.num_rel, self.config.emb_dim)
        self.rel_4 = nn.Embedding(self.config.num_rel, self.config.emb_dim)
        self.rel_5 = nn.Embedding(self.config.num_rel, self.config.emb_dim)
        self.rel_6 = nn.Embedding(self.config.num_rel, self.config.emb_dim)
        self.rel_7 = nn.Embedding(self.config.num_rel, self.config.emb_dim)
        self.rel_8 = nn.Embedding(self.config.num_rel, self.config.emb_dim)
        self.rel_w = nn.Embedding(self.config.num_rel, self.config.emb_dim)
        self.criterion = nn.Softplus()
        self.fc = nn.Linear(100, 50, bias=False)
        self.ent_dropout = torch.nn.Dropout(0)
        self.rel_dropout = torch.nn.Dropout(0)
        self.bn = torch.nn.BatchNorm1d(0)
        self.init_weights()

    def init_weights(self):
        if True:
            r, i, j, k,r_1,i_1,j_1,k_1 = self.quaternion_init(self.config.num_ent, self.config.emb_dim)
            ...
            s, x, y, z,s_1,x_1,y_1,z_1 = self.quaternion_init(self.config.num_ent, self.config.emb_dim)
